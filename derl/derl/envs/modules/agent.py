...
try:
    ...
    import sys
    sys.path.append("..")
    import derl.derl.utils.camera as cu
...
class Agent:
    ...
    def observation_step(self, env, sim):
        ...
        # print(f"proprioceptive: {np.concatenate(observations).ravel().shape}")
        return {
            "proprioceptive": np.concatenate(observations).ravel(),
            # "body_mass": sim.model.body_mass[self.agent_body_idxs].copy()[:, np.newaxis],
        }
