...
class PPO:
    def __init__(self, xml_file=None):
        ...
        if cfg.PPO.CHECKPOINT_PATH:
            ob_rms = self.restore_from_checkpoint(self.actor_critic)
    ...
    def train(self, exit_cond=None):
        ...
        for cur_iter in range(cfg.PPO.MAX_ITERS):
            ...
            # if len(ep_rew["reward"]):
            #     cur_rew = np.mean(ep_rew["reward"])
    ...
    def restore_from_checkpoint(self, ac):
        print("Loading pretrained actor critic model")
        model_p, ob_rms = torch.load(cfg.PPO.CHECKPOINT_PATH, map_location="cuda:0")
